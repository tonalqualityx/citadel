...
6. **[NEW]** Project team assignments (function → user mapping)
...
Tasks have a **Requirements** checklist—a simple list of checkbox items that must be completed as part of the task.
...
│  âš”️                 │
│                     │
│  sop_id: [ref]      │  ← links back to source SOP
│  requirements:      │  ← copied from template
│  â˜ Step 1           │
│  â˜ Step 2           │
│  â˜ Step 3           │
│  â˜ Step 4 (added)   │  ← user can add more
└─────────────────────â”˜
```
...
**Location:** Settings → Preferences → Naming Convention
...
| Task | Quest | âš”️ | `tasks` |
| SOP | Rune | 📜 | `sops` |
| Recipe/Template | Scroll | 👨‍🍳 | `recipes` |
| Domain | Domain | 💨 | `domains` |
| Tool | Tool | 📧 | `tools` |
| Function | Function | 💤 | `functions` |
| Time Entry | Time Entry | â±️ | `time_entries` |
| Team Member | Guild Member | 💥 | `users` |
| Meeting | Council | 🏣️ | `meetings` |
| Note | Note | 📝 | `notes` |
...
| Exports/reports | âš™️ Configurable |
...
│  priority              VARCHAR(50)                              │
│  energy_base           DECIMAL                                  │
│  energy_mystery_factor VARCHAR(50)                              │
│  due_date              DATE                                     │
│  phase                 VARCHAR(50)                              │
│                                                                 │
│  project_id            UUID REFERENCES projects(id)             │
│  site_id               UUID REFERENCES sites(id)                │
│  sop_id                UUID REFERENCES sops(id) NULLABLE        │  ← Source template
│  function_id           UUID REFERENCES functions(id) NULLABLE   │
...
│  completed_by          UUID REFERENCES users(id) NULLABLE       │
│  sort_order            INTEGER                                  │
│                                                                 │
│  created_at            TIMESTAMP                                │
│  updated_at            TIMESTAMP                                │
...
│  name                  VARCHAR(255)                             │
│  description           TEXT                                     │
│  instructions          TEXT                                     │  ← Rich text content
│  tags                  VARCHAR[] ARRAY                          │
│  last_reviewed         DATE                                     │
│                                                                 │
│  owner_function_id     UUID REFERENCES functions(id) NULLABLE   │
│                                                                 │
│  created_at            TIMESTAMP                                │
│  updated_at            TIMESTAMP                                │
└─────────────────────────────────────────────────────────────────â”˜
...
│                                                                 │
│  naming_convention     VARCHAR(20) DEFAULT 'awesome'            │
...
When creating a project from a Recipe, users define the website's pages (sitemap). This drives variable task generation—for example, creating one "Build Page" task per page, or one "Mockup" task per page that needs a unique design.
...
│  needs_unique_mockup     BOOLEAN DEFAULT true                    │
│  mockup_template_page_id UUID REFERENCES project_pages(id)       │
...
```
Page: "SEO Services"
├── needs_unique_mockup: false
├── mockup_template_page_id: → "Services" page
└── Behavior: 
...
├── Function: Project Manager    → User: Sarah Jenkins
├── Function: CSA                → User: Emily Chen
├── Function: Content Writer     → User: Alex Rivera
├── Function: Designer           → User: (unassigned)
├── Function: WordPress Tech     → User: (unassigned)
├── Function: Network Admin      → User: Tom Wilson
└── Function: AR                 → User: Finance Team
```
...
├── "Design Approval" - 25% ($1,500)
│   └── trigger_type: task_completion
│   └── trigger_task_id: → "Client approval on complete design" task
├── "Build Approval" - 50% ($3,000)
│   └── trigger_type: task_completion
│   └── trigger_task_id: → "Client approval on staging site" task
```
...
Team members can have multiple functions (roles). This allows filtering when assigning people to project functions—only show users who have the Designer function when assigning the Designer role.
...
```
User: Sarah Jenkins
├── Function: Project Manager (primary)
└── Function: CSA

User: Mike Hansen  
├── Function: Designer (primary)
└── Function: WordPress Tech

User: Alex Rivera
└── Function: Content Writer (primary)
```
...
│  requires_sitemap        BOOLEAN DEFAULT false           [NEW]   │
│  estimated_tasks         INTEGER                         [NEW]   │
...
│  function_id                 UUID REFERENCES functions(id) [NEW] │
│  name                        VARCHAR(255) NOT NULL               │
│  phase                       VARCHAR(50) NOT NULL                │
│  order                       INTEGER NOT NULL                    │
│  task_type                   VARCHAR(20) NOT NULL                │
│  is_milestone                BOOLEAN DEFAULT false        [NEW]  │
│  default_billing_percentage  DECIMAL                      [NEW]  │
...
│  ...existing fields...                                           │
│                                                                  │
│  billing_type              VARCHAR(20)                   [NEW]   │
│  budget_hours              DECIMAL                       [NEW]   │
│  hourly_rate               DECIMAL                       [NEW]   │
│  budget_amount             DECIMAL                       [NEW]   │
│                                                                  │
│  ...existing fields...                                           │
└─────────────────────────────────────────────────────────────────â”˜
```
...
```
â”Œ─────────────────────────────────────────────────────────────────────────────────â”
│                    PACT CREATION WORKFLOW ENTITIES                               │
└─────────────────────────────────────────────────────────────────────────────────â”˜
...
└──────────────â”˜             └──────â”¬───────â”˜             └──────────────â”˜
         ...
         ▼                           ▼                           ▼
â”Œ──────────────────â”         â”Œ──────────────────â”         â”Œ──────────────â”
│  PROJECT_PAGES   │         │ PROJECT_BILLING  │         │    TASKS     │
│     (sitemap)    │         │   _MILESTONES    │         │     âš”️       │
└──────────────────â”˜         └──────────────────â”˜         └──────────────â”˜
         │                           │
         │ drives generation         │ triggers
         └───────────────────────────â”´──────────────â–º (invoices - future)
```
...
- `indelible-wireframes-quest-detail.md` — Quest/Task detail view
- `indelible-wireframes-list-views.md` — List view wireframes
