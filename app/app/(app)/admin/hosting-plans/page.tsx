...
import {
  useHostingPlans,
  useCreateHostingPlan,
  useUpdateHostingPlan,
  useDeleteHostingPlan,
...
import type { HostingPlan, CreateHostingPlanInput } from '@/types/entities';
...
export default function HostingPlansAdminPage() {
  ...
  const [editingItem, setEditingItem] = React.useState<HostingPlan | null>(null);
  const [deleteConfirm, setDeleteConfirm] = React.useState<string | null>(null);
  const [deleteError, setDeleteError] = React.useState<string | null>(null);

  const { data, isLoading, error } = useHostingPlans(includeInactive);
  const createMutation = useCreateHostingPlan();
  const updateMutation = useUpdateHostingPlan();
  const deleteMutation = useDeleteHostingPlan();

  const [formData, setFormData] = React.useState<CreateHostingPlanInput>({
    name: '',
    rate: 0,
    agency_rate: null,
    monthly_cost: null,
  ...
  const openEditModal = (item: HostingPlan) => {
    setEditingItem(item);
    setFormData({
      name: item.name,
      rate: item.rate,
        ...
        <Card>
          <CardContent className="py-8">
            <EmptyState
              ...
              title="Error loading hosting plans"
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-text-main">Hosting Plans</h1>
          <p className="text-text-sub">Manage hosting plan tiers and pricing</p>
        </div>
      ...
      <Card>
        <CardContent className="p-0">
          ...
          ) : data?.hosting_plans.length === 0 ? (
            <EmptyState
              icon={<span className="text-4xl">🏠</span>}
              title="No hosting plans found"
              description="Get started by adding your first hosting plan"
              action={
                <Button onClick={openCreateModal}>
                  <Plus className="h-4 w-4 mr-2" />
                  Add Plan
              ...
              <tbody className="divide-y divide-border-warm">
                {data?.hosting_plans.map((plan) => (
                  <tr key={plan.id} className="hover:bg-background-light/50">
                    ...
                    <td className="px-4 py-3">
                      <span className="text-sm text-text-sub">{plan.vendor_plan || '-'}</span>
                    </td>
      ...
      <Modal open={isModalOpen} onOpenChange={setIsModalOpen}>
        <ModalContent size="lg">
          <form onSubmit={handleSubmit}>
            <ModalHeader>
              <ModalTitle>{editingItem ? 'Edit Hosting Plan' : 'Add Hosting Plan'}</ModalTitle>
            </ModalHeader>
            <ModalBody className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        monthly_cost: e.target.value ? parseFloat(e.target.value) : null,
                      })
                    }
                  />
                </div>
                <div>
                  ...
                  <Input
                    value={formData.vendor_plan || ''}
                    onChange={(e) => setFormData({ ...formData, vendor_plan: e.target.value })}
                    placeholder="e.g., Cloudways Basic"
                  />
                </div>
                <div className="col-span-2">
      ...
      <Modal open={!!deleteConfirm} onOpenChange={() => { setDeleteConfirm(null); setDeleteError(null); }}>
        <ModalContent>
          <ModalHeader>
            <ModalTitle>Delete Hosting Plan</ModalTitle>
          </ModalHeader>
          <ModalBody>
            <p className="text-text-sub">
              Are you sure you want to delete this hosting plan? This action cannot be undone.
            </p>
            {deleteError && (
