...
import { useMaintenancePlans } from '@/lib/hooks/use-reference-data';
import type { MaintenancePlan } from '@/types/entities';

interface MaintenancePlanSelectProps {
  value: string | null;
...
export function MaintenancePlanSelect({
  ...
  const selectedPlan = plans.find((p: MaintenancePlan) => p.id === value);
  const displayLabel = selectedPlan?.name || placeholder;

  const filteredPlans = plans.filter((p: MaintenancePlan) =>
        ...
        <div className="absolute z-50 mt-1 w-64 bg-surface border border-border rounded-lg shadow-lg">
          ...
          <div className="max-h-48 overflow-auto py-1">
            ...
            {isLoading ? (
              ...
              filteredPlans.map((plan: MaintenancePlan) => (
                <button
                  key={plan.id}
                  type="button"
                  onClick={() => {
                ...
                >
                  ...
                  <div className="text-xs text-text-sub">
                    ${Number(plan.rate).toFixed(2)}/mo - {formatFrequency(plan.frequency)}
                  </div>
                </button>
              ))
            )}
